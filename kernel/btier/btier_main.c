...
/* No space on the device to copy to is not an error */
	if (0 == newdevice->device)
		return 0; // should check this . Xiongzi Ge
...
int migrate_file_down(struct tier_device *dev, u64 blocknr_start, u64 offset)
{
    ...
    {
      blocknr_start += 1; 
      binfo = get_blockinfo(dev, blocknr_start, 0);
	
	       if (!binfo)
		   goto end_error;
	
	      //orgdevice=binfo.device;
           destdevice=1;/*as file down*/
	
	      if (binfo->device == destdevice + 1) {
	      res = -EEXIST;
		  pr_err("Failed to migrate block %llu, already on device %i\n",
		       blocknr_start, destdevice);
		  goto end_error_free;
	      }
	      //tier_sync(dev);
	      ...
	      if (!orgbinfo) {
	       ...
	       binfo->device = destdevice + 1;
	
	       res = copyblock(dev, binfo, orgbinfo, blocknr_start);
	       if (res) {
			reset_counters_on_migration(dev, orgbinfo);
			clear_dev_list(dev, orgbinfo);
			discard_on_real_device(dev, orgbinfo);
	      ...
	      if (dev->migrate_verbose)
			pr_info("file has been migrated down..local -> cloud..\n");
	      end_error_free:
	      kfree(binfo);
		 }
...
static void data_migrator(struct work_struct *work)
{
	...
	while (!dev->stop) {
		...
		/*do dev->migrate for the timer based migration for a bunch of blocks */
                btier_lock(dev);
		tier_sync(dev);
		walk_blocklist(dev);
                btier_unlock(dev);
