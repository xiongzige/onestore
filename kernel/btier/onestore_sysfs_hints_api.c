...
/* 
 * Set up the Access Hints Interface for the applications
 * First usage case  1 file, with the migration up/down 
   "fd, 0" 
    return true afte the migration

    Xiongzi Ge : xiongzige@gmail.com
   
  */
static ssize_t tier_attr_migrate_hints_store(	struct tier_device *dev,
							 const char *buf, size_t s)
{
		u64 blocknr;
		int device;
...
/* 
 * Migrate a file up/down

    Xiongzi Ge : xiongzige@gmail.com
   
  */
static ssize_t tier_attr_migrate_file_down_store(	struct tier_device *dev,
							 const char *buf, size_t s)
{
		u64 blocknr_start;
		u64 blocknr_offset;
		...
		res = sscanf( cpybuf, "%d/",&extent_num);

		for(i=0;i<extent_num; i++)
			{
			 		res = sscanf(cpybuf+2+i*17*2, "%llx/%llx/", &blocknr_start, &blocknr_offset);
		            if (res != 2)
			        goto end_error;
		                blocknr_start = blocknr_start >> 20;
                                blocknr_offset = blocknr_offset >> 20;
		            if ( blocknr_start+blocknr_offset-1< maxblocks ) {
					res = migrate_file_down(dev, blocknr_start, blocknr_offset);
					if (res < 0)
						s = res;
					else
						s = m;
		...
		if (blocknr +1 < maxblocks) {
			res = migrate_direct(dev, blocknr+1, device);
			if (res < 0)
				s = res;
			else
				s = m;
	...
	static ssize_t tier_attr_migrate_file_test_store(	struct tier_device *dev,
								 const char *buf, size_t s)
	{
			u64 blocknr_start;
			u64 blocknr_offset;
			...
			//for(i=0;i<extent_num; i++)
				{
						res = sscanf(cpybuf, "%llu/%llu/%llu", &blocknr_start, &blocknr_offset,&u64test);
						//if (res != 2)
						//goto end_error;
			                        pr_err("%llu", u64test>>20);
                                                blocknr_start=blocknr_start>>20;
                                                blocknr_offset=blocknr_offset>>20;
						if ( blocknr_start+blocknr_offset-1< maxblocks ) {
							res = migrate_file_down(dev, blocknr_start, blocknr_offset);
...
TIER_ATTR_WO(migrate_hints);
TIER_ATTR_WO(migrate_file_down);
TIER_ATTR_WO(migrate_file_test);
...
struct attribute *tier_attrs[] = {
	...
	&tier_attr_migrate_hints.attr,
	&tier_attr_migrate_file_down.attr,
	&tier_attr_migrate_file_test.attr,
	NULL,
};
